...
#   OPENSHELL_VERSION     — OpenShell CLI release tag (default: v0.0.47)
...
# ── Configuration ────────────────────────────────────────────────────
OPENSHELL_VERSION="${OPENSHELL_VERSION:-v0.0.47}"
...
if [[ "${SKIP_DOCKER_PULL:-0}" != "1" ]]; then
  ...
  (
    SUPERVISOR_TAG="${OPENSHELL_VERSION#v}" # v0.0.47 -> 0.0.47
