...
import {
  ...
  getDockerDriverGatewayJwtMaterial,
...
describe("buildDockerGatewayDebEnvFile", () => {
  it("replaces all managed gateway env keys and preserves unrelated values", () => {
    ...
    expect(next).toContain("OPENSHELL_DISABLE_GATEWAY_AUTH=false\n");
    expect(next).toContain("OPENSHELL_LOCAL_TLS_DIR=/tmp/tls\n");
  ...
  it("creates complete gateway JWT material for Docker-driver sandbox auth", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-gateway-state-"));
    try {
      const material = ensureDockerDriverGatewayJwtMaterial(stateDir);

      expect(material).toEqual(getDockerDriverGatewayJwtMaterial(stateDir));
      expect(fs.readFileSync(material.signingKeyPath, "utf-8")).toContain("PRIVATE KEY");
      expect(fs.readFileSync(material.publicKeyPath, "utf-8")).toContain("PUBLIC KEY");
      expect(fs.readFileSync(material.kidPath, "utf-8").trim()).toMatch(/^[0-9a-f]{32}$/);
      expect(fs.statSync(material.signingKeyPath).mode & 0o777).toBe(0o600);
      expect(fs.statSync(path.dirname(material.signingKeyPath)).mode & 0o777).toBe(0o700);
    } finally {
